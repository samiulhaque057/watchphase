...
import { StockAdminPanel } from "./stock-admin-panel";
...
export type AdminListedProduct = Prisma.ProductGetPayload<{
  select: typeof productListSelect;
}>;

export type AdminListedListingCategory = {
  id: string;
  slug: string;
  label: string;
  listingTag: string;
...
export const metadata: Metadata = {
  ...
  description: "Internal inventory tooling.",
...
export default async function SecretStockAdminPage({
  params,
  searchParams,
}: PageProps) {
  ...
  if (sp.password != null) {
    // Never allow credentials in query string; normalize URL immediately.
    redirect(`/internal/stock/${secret}`);
  }
  const configured = process.env.ADMIN_ROUTE_KEY;
